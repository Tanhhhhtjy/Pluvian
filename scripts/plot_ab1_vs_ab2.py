...
def main(ckpt_ab1, ckpt_ab2, out_png, start="2023-08-08T06:00:00", mc=False):
    print(f"=== ab1 vs ab2 4-row figure  start={start}  mc={mc} ===")
    ...
    with torch.no_grad():
        if mc:
            # single stochastic MC-dropout sample (sharper than the
            # deterministic mean, which is blurry by construction)
            pred_ab1 = ab1.mc_dropout_predict(make_batch(sample, device), n_samples=1)[0, 0].cpu().numpy()[:, :Ht, :Wt]
            pred_ab2 = ab2.mc_dropout_predict(make_batch(sample, device), n_samples=1)[0, 0].cpu().numpy()[:, :Ht, :Wt]
        else:
...
if __name__ == "__main__":
    ...
    main(pos[0], pos[1], pos[2], pos[3] if len(pos) > 3 else "2023-08-08T06:00:00", mc=mc)
